...
function PlaylistPage() {
  ...
  const handleLogin = () => {
    ...
    const url = `https://accounts.spotify.com/authorize?client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUrl)}&scope=${scope}&response_type=token&show_dialog=true`;
  ...
  const handleCreatePlaylist = async () => {
    ...
    try {
      ...
      if (playlistCover) {
        ...
        await spotifyApi.uploadCustomPlaylistCoverImage(playlist.id, base64Cover);
      }
  ...
  const handleGenerateTracks = async () => {
    try {
      ...
      const query = `${params.genre ? 'genre:' + params.genre : ''} ${params.year ? 'year:' + params.year : ''}`;
      ...
      {!isLoggedIn ? (
        ...
        <div className="pl-form-container">
          <div className="pl-param-container">
            ...
            <label>
              Genre:
              <input type="text" name="genre" value={params.genre} onChange={handleParamChange} />
            </label>
            <label>
              Year:
              <input type="text" name="year" value={params.year} onChange={handleParamChange} />
            </label>
            <label>
              Acousticness:
              <input type="range" name="acousticness" min="0" max="1" step="0.01" value={params.acousticness} onChange={handleSliderChange} />
              <span>{params.acousticness}</span>
            </label>
            <label>
              Danceability:
              <input type="range" name="danceability" min="0" max="1" step="0.01" value={params.danceability} onChange={handleSliderChange} />
              <span>{params.danceability}</span>
            </label>
            <label>
              Energy:
              <input type="range" name="energy" min="0" max="1" step="0.01" value={params.energy} onChange={handleSliderChange} />
              <span>{params.energy}</span>
            </label>
            <label>
              Instrumentalness:
              <input type="range" name="instrumentalness" min="0" max="1" step="0.01" value={params.instrumentalness} onChange={handleSliderChange} />
              <span>{params.instrumentalness}</span>
            </label>
            <label>
              Speechiness:
              <input type="range" name="speechiness" min="0" max="1" step="0.01" value={params.speechiness} onChange={handleSliderChange} />
              <span>{params.speechiness}</span>
            </label>
            <label>
              Valence:
              <input type="range" name="valence" min="0" max="1" step="0.01" value={params.valence} onChange={handleSliderChange} />
              <span>{params.valence}</span>
            </label>
            <label>
              Tempo:
              <input type="range" name="tempo" min="0" max="250" step="1" value={params.tempo} onChange={handleSliderChange} />
